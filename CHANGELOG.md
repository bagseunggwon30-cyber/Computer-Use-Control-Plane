...
- **`references\live-cassette-runner.ps1`** (interactive runner):
  - Notepad / Electron app / Chrome / native desktop app 4환경 cassette 한 번에 보존.
  - 결과: `live-verify\<env>\*.json` (`cucp.live-verify/v1` schema).
  - 사용법: `.\references\live-cassette-runner.ps1 -Env all` (또는 `notepad` / `electron` / `chrome` / `desktop`).
...
- **cassette runner 는 사용자 라이브 환경 의존**: Notepad / Electron app / Chrome / native desktop app
...
- **4환경 cassette 대상** (별도 단계 필요): Notepad / Electron app / Chrome / native desktop app.
...
- **mouse-verify 정확도 통계는 코드 준비만**: 실제 4환경 cassette (Notepad / Electron app /
...
skeleton (437라인) 위에 직접 통합하는 형식. AI agent (Codex / Claude / Electron app / 일반
...
CUCP 가 Electron 앱 (Electron app / VS Code / Slack / Discord / Notion / Postman 등 chromium 기반)
...
|---|---|---|
| Electron app 입력란 좌표 정확한데 textarea focus 안 잡힘 | UIA tree 에 contenteditable 미노출 | `element.focus()` 한 줄 |
| 메이플 Chrome 이 Electron app 위에 떠서 hit-test 가 다른 윈도우 잡음 | 좌표 기반 본질적 한계 | DOM 은 윈도우 layout 무관 |
| Electron app 가 창모드 ↔ 전체화면 토글 시 좌표 의미 잃음 | 좌표 기반 본질적 한계 | DOM 좌표 계산 안 함 |
...
- **`-Action cdp-eval -CdpExpr "<js>" [-CdpPageMatch Electron app]`** (read-only) — JS 실행
...
- **`references/cdp-setup.md`** 신규 — Electron app / VS Code / Slack 활성화 가이드
...
| ⚠️ Electron app 재시작 필요 (런타임 토글 안 됨) | 한 번 작업 끊김 |
| ❌ Native Win32 앱 (메모장, settings panels) | CDP 미지원 → Stage 1~6 fallback |

### 솔직 평가

**라이브 검증 안 됨** — 9222 포트 닫혀있는 상태에서 코드 + 테스트만 통과. 실제
Electron app DOM 직접 입력은 사용자가 `--remote-debugging-port=9222` 옵션 추가해서 Electron app
...
라이브 사고 두 가지 ((a) 코드 에디터 의도치 않은 입력, (b) Electron app 전체화면 → 창모드
...
이걸로 v0.7.0 의 Electron app send 화살표 사례에서 마지막 남은 두 가지 약점을 닫았다:
...
### Electron app send 화살표 사례에서 발견된 정확도 문제 해결

전 sprint에서 Codex가 Electron app 보라색 send 화살표를 못 찾고 여러 번 빗나가다가
...
| ✅ Electron 앱 (Codex/Electron app/Discord 등) | UIA 트리 노출됨, 동작 |
