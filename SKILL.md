...
description: |
  Use the local CUCP (Computer Use Control Plane) CLI from any Codex / Claude / Electron app project to observe, ground, and operate the user's Windows desktop at Claude Computer Use grade. Single entry point is scripts/cucp.ps1. Trigger on cucp, CUP, computer use, computer-use, Windows control, desktop control, appshot, snapshot, live benchmark, desktop benchmark, screen control, GUI automation, PC/app control, or whenever the user asks Codex to inspect, click, type, drag, scroll, switch apps, follow a goal, or autonomously operate the local Windows desktop. v2.3.0 macros include version, daemon, mouse-verify, cdp-prosemirror-insert, cdp-deep-find, modal-detect, recovery-plan, recovery-run, precision-validate, benchmark, release-notes, ime-paste, safe-type-ime, recorder, audit-summary, and policy-check.
---
...
## Low-lag operating pattern (Codex + Electron app 동시 사용)

긴 세션에서 Codex/Electron app 데스크톱 앱이 느려질 수 있어요. 권장:
...
& <wrapper> macro smart-plan --label "Save" --match "Electron app" --precision-points  # planner can recommend micro-refined click-point route
...
& <wrapper> macro workflow-run --dry-run --step "macro hit-test --x 1200 --y 720 --fast" --step "macro click-point --x 1200 --y 720 --target-match Electron app"  # gated macro sequence dry-run
...
& <wrapper> macro hit-test --x 1200 --y 720 --target-match Electron app --fast  # fast Win32-only point guard
& <wrapper> macro hit-test-batch --points "1200,720;1210,720;1220,720" --target-match Electron app  # fast multi-point guard
& <wrapper> macro hit-scan --x 1200 --y 720 --radius 4 --step 2 --target-match Electron app  # read-only micro coordinate scan
& <wrapper> macro coord-profile --x 1200 --y 720 --target-match Electron app  # read-only DPI/monitor/window coordinate profile
& <wrapper> macro coord-map --from window --x 40 --y 24 --target-match Electron app  # read-only window/screen/normalized coordinate transform
& <wrapper> macro coord-anchor --x 1200 --y 720 --target-match Electron app  # read-only reusable layout-relative coordinate anchor
& <wrapper> macro coord-anchor --x 1200 --y 720 --target-match Electron app --record-history  # optionally remember verified anchors for reuse scoring
& <wrapper> macro point-plan --x 1200 --y 720 --radius 6 --step 2 --target-match Electron app --cache-ttl 2  # read-only precision click plan with short TTL cache
& <wrapper> macro target-validate --x 1200 --y 720 --target-match Electron app --min-confidence medium  # read-only pre-click small target safety check
...
& <wrapper> -AllowLiveControl macro smart-click --label "Save" --match "Electron app" --allow-mouse-fallback --precision-points  # cascade click with micro-refined UIA coordinate fallback
& <wrapper> -AllowLiveControl macro click-point --x 1200 --y 720 --target-match Electron app --refine uia-safe  # guarded raw coordinate click
& <wrapper> -AllowLiveControl macro click-point --x 1200 --y 720 --target-match Electron app --micro-refine --precision-radius 6 --precision-step 2  # live click with pre-click micro scan
...
& <wrapper> macro diagnose-lag --sample-ms 3000   # Codex/Electron app/Chrome/node 프로세스 + 경고
