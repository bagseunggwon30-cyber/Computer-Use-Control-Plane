...
CUCP v1.3.0 부터 Electron 앱 (Electron app / VS Code / Slack / Discord 등) 의 DOM 직접
...
대부분의 Electron app은 기본적으로 `--remote-debugging-port` 가 꺼진 상태로 실행됩니다.
다음 두 가지 방법 중 하나 사용:

### 방법 1 — 단축키 / 바탕화면 아이콘 수정 (권장)

1. 대상 Electron app 바로가기 (예: 시작메뉴, 작업표시줄) 우클릭 → **속성**
2. **대상(Target)** 필드 끝에 ` --remote-debugging-port=9222` 추가
   ```
   "C:\Path\To\Your\ElectronApp.exe" --remote-debugging-port=9222
   ```
3. 적용 → 다음 실행부터 9222 포트로 CDP 가능
...
대상 Electron app 의 DOM 에 접근 가능**합니다. 즉:

- 같은 PC 의 다른 사용자/프로세스가 대상 앱 내용을 읽을 수 있음
- 같은 PC 의 다른 사용자/프로세스가 대상 앱에 명령을 입력할 수 있음
...
# 대상 Electron app 페이지에서 임의 JS 실행
& <wrapper> macro cdp-eval --expr "document.title" --page-match "Target App"

# 대상 앱 입력란에 텍스트 + Enter (좌표 무관)
& <wrapper> -AllowLiveControl macro cdp-type \
   --selector "textarea[placeholder*='Questions or describe']" \
   --text "Hello from CUCP" \
   --press-enter \
   --page-match "Target App"
...
& <wrapper> -AllowLiveControl macro smart-click --label "Send" --cdp-page-match "Target App"
