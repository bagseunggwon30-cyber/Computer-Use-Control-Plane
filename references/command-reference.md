...
```powershell
& C:\Users\K\.codex\skills\cucp-computer-use\scripts\cucp.ps1 macro find-label --label "확인" --window "Notepad"
```

옵션: `--label` (필수), `--window` (제목 일부), `--match` (appshot 필터, 기본은 window), `--role` (button/edit/etc).

`-Brief`를 붙이면 한 줄 요약: `ok find-label '확인' @(120,80) win='Notepad'`.
...
```powershell
& C:\Users\K\.codex\skills\cucp-computer-use\scripts\cucp.ps1 -AllowLiveControl macro focus-window --name "Notepad"
```
...
```powershell
& C:\Users\K\.codex\skills\cucp-computer-use\scripts\cucp.ps1 macro wait-window --title "Notepad" --timeout-ms 10000 --interval-ms 500
```
...
```powershell
& C:\Users\K\.codex\skills\cucp-computer-use\scripts\cucp.ps1 macro wait-label --label "다운로드 완료" --window "Notepad" --timeout-ms 30000
```
...
& <wrapper> macro native-windows --match Electron app
...
& <wrapper> -AllowLiveControl macro click-point --x 500 --y 300 --target-match Electron app --refine uia-safe --click-inset 2
& <wrapper> -AllowLiveControl macro click-point --x 500 --y 300 --target-match Electron app --micro-refine --precision-radius 6 --precision-step 2 --cache-ttl 2
& <wrapper> -AllowLiveControl macro click-point --x 500 --y 300 --target-match Electron app --no-micro-refine
...
& <wrapper> -AllowLiveControl macro uia-click-label --label "최소화" --match "Electron app"
```
...
& <wrapper> macro smart-plan --label "Send" --allow-cdp --cdp-page-match Electron app
& <wrapper> macro smart-plan --label "Save" --match "Electron app" --precision-points --precision-radius 6 --precision-step 2
...
& <wrapper> macro smart-plan --label "확인" --match "Electron app" --include-ocr
...
& <wrapper> macro app-profile --match "Notepad" --label "OK" --probe-uia --include-affordances --json-only
& <wrapper> macro app-profile --match "Electron app" --label "Run" --auto-probe --record-strategy --json-only
& <wrapper> macro app-profile --label "Save"
```

Output schema is `cucp.app-profile/v1`. Important fields:
- `app_type`: `browser_or_electron`, `document_or_mail_app`, or `win32_desktop`.
...
- `route_order`: recommended control order, for example CDP/DOM first for Chrome/Electron, UIA first for desktop apps, and guarded precision points when labels are not exposed.
...
& <wrapper> macro workflow-run --dry-run --step "macro hit-test --x 1200 --y 720 --fast" --step "macro click-point --x 1200 --y 720 --target-match Electron app"
...
& <wrapper> -AllowLiveControl macro smart-click --label "Save" --match "Electron app" --allow-mouse-fallback --precision-points --precision-radius 6 --precision-step 2
...
& <wrapper> macro cdp-eval --expr "document.title" --page-match Electron app
& <wrapper> macro cdp-eval --expr-b64 <utf8-base64-javascript> --page-match Electron app
& <wrapper> macro cdp-smart-find --text "Send" --page-match Electron app
& <wrapper> macro cdp-smart-type-find --label "Message" --page-match Electron app
& <wrapper> -AllowLiveControl macro cdp-click --selector "button[aria-label='Send']" --page-match Electron app
& <wrapper> -AllowLiveControl macro cdp-type --selector "textarea" --text "hello" --press-enter --page-match Electron app
& <wrapper> -AllowLiveControl macro cdp-smart-click --text "Send" --page-match Electron app
& <wrapper> -AllowLiveControl macro cdp-smart-type --label "Message" --text "hello" --press-enter --page-match Electron app
```
...
& <wrapper> macro ocr-find-text --text "Send" --target-match "Electron app"
...
& <wrapper> macro coord-profile --x 1200 --y 720 --target-match Electron app
& <wrapper> macro coord-map --from window --x 40 --y 24 --target-match Electron app
& <wrapper> macro coord-map --from normalized --norm-x 0.5 --norm-y 0.5 --target-match Electron app
& <wrapper> macro coord-anchor --x 1200 --y 720 --target-match Electron app
& <wrapper> macro coord-anchor --x 1200 --y 720 --target-match Electron app --record-history
& <wrapper> macro hit-test --x 1200 --y 720 --target-match Electron app --click-inset 3
...
& <wrapper> macro hit-test --x 1200 --y 720 --target-match Electron app --fast
# JSON 출력의 source=wrapper_win32_fast, uia_skipped=true

# read-only: 여러 후보 좌표를 한 번에 빠르게 검사. AI가 작은 버튼 후보를 비교할 때 사용
& <wrapper> macro hit-test-batch --points "1200,720;1210,720;1220,720" --target-match Electron app
...
& <wrapper> macro hit-scan --x 1200 --y 720 --target-match Electron app
& <wrapper> macro hit-scan --x 1200 --y 720 --radius 4 --step 2 --target-match Electron app --click-inset 2
& <wrapper> macro point-plan --x 1200 --y 720 --radius 6 --step 2 --target-match Electron app --cache-ttl 2
& <wrapper> macro target-validate --x 1200 --y 720 --target-match Electron app --min-confidence medium
...
icon, or narrow custom-drawn control.
...
& <wrapper> macro ocr-uia-fuse --text "전송" --match-window "Electron app" --language ko
...
```powershell
& <wrapper> macro precision-validate --x 1200 --y 720 [--target-match Electron app] [--samples 5]
```
